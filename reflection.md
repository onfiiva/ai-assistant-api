...
## 4 занятие
### Что понял
- Как организовать структуру с rate limit через Redis, конфигами в Vault, обернуть в Docker
- Что у бесплатных ключей есть не только лимит запросов в минуту, но и в день :)

### Что было сложным
- Перелопатить конфиги, поднять в контейнеры и протестировать весь функционал целиком

### Какие вопросы остались
- Насколько устойчивая инфраструктура все-таки вышла

## 5 занятие
### Что понял
- Что можно кэшировать через Redis еще и ответы от API модели, чтобы однотипные ответы возвращать без фактичнского запроса к API и не тратить rate limit
- Поизучал метрики prometheus для сего волшебства
...
## 1 занятие
### Что понял
- Каким образом работает подбор правильных токенов у модели для продолжения текста
- Почему такие цены на память
- Как вручную работать с определенными документами и сделать модель анлогом секретаря с функцией пересказа

### Что было сложным
- Вспомнить векторную математику

### Какие вопросы остались
- Насколько применяется RAG в аспекте LLM engineering, и на каких примерах